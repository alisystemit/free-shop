...
<div class="wrapper">
    <div id="login-div">
        <div class="login-div-name">
            ...
            <div class="login-div-123"></div>
            <div class="login-div-123"></div>
            <div class="login-div-txt"> ورود مدیر به سایت </div>
        </div>
        <form name="login" action="<?= '//'.$_SERVER['SERVER_NAME'].'/'.DOCUMENT_ROOT.DOCUMENT_slash ?>admin/login/check" method="post">
            <img src="<?= '//'.$_SERVER['SERVER_NAME'].'/'.DOCUMENT_ROOT.DOCUMENT_slash ?>theme/img/ava.png" alt="" class="login-avatar" />
            <input type="hidden" name="crf" value="<?php echo $x=md5(mt_rand(0,1000000)); $_SESSION['cfr'] = $x ; ?>">
            <input type="text" name="username" placeholder="نام کاربری را وارد کنید" required >
            <input type="password" name="password" placeholder="پسورد خود را وارد کنید" required >
            <?php
